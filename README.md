...
Утилита для **анализа** .exe‑файлов, собранных через **Nuitka**, но может и **Pyinstaller**. Извлекает метаданные, строки, модули, информацию о PE‑структуре и другую полезную информацию.
...
>* Pyinstaller выдаёт более подробную информацию, т.к. он устроен проще, чем Nuitka.
>* Не предназначен для работы с PyInstaller, cx_Freeze и аналогичными упаковщиками, но умеет с ними работать. Всё равно приоритет на Nuitka.
